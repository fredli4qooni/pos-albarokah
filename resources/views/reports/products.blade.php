...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-slate-500 mb-1">
                ...
                <span>/</span>
                <span class="text-slate-800 font-semibold">Laporan Stok & Restok SMA</span>
            </div>
            <h1 class="text-2xl lg:text-3xl font-black text-slate-900 tracking-tight">Laporan Inventori & Rekomendasi Restok</h1>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
        <form method="GET" action="{{ route('reports.products') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wider">Kategori Produk</label>
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wider">Kondisi & Status Stok</label>
            ...
            <div class="flex items-center gap-2">
                <button type="submit" 
                        class="flex-1 px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white text-sm font-bold rounded-xl shadow-sm transition">
                    Terapkan Filter
                </button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Valuasi Modal Inventori</span>
            <div class="mt-2 text-2xl font-black text-teal-700 font-mono">
                Rp {{ number_format($summary['total_purchase_valuation'], 0, ',', '.') }}
            </div>
            <span class="text-xs text-slate-500 mt-1 block">Berdasarkan harga beli modal</span>
        </div>

        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Estimasi Nilai Jual</span>
            <div class="mt-2 text-2xl font-black text-emerald-700 font-mono">
                Rp {{ number_format($summary['total_selling_valuation'], 0, ',', '.') }}
            </div>
            <span class="text-xs text-slate-500 mt-1 block">Potensi perputaran kas</span>
        </div>

        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Total Kuantitas Fisik</span>
            <div class="mt-2 text-2xl font-black text-slate-900 font-mono">
                {{ number_format($summary['total_units']) }} <span class="text-sm font-normal text-slate-500">Unit</span>
            </div>
            <span class="text-xs text-slate-500 mt-1 block">Dari {{ number_format($summary['total_products']) }} varian SKU</span>
        </div>

        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Peringatan Restok</span>
            <div class="mt-2 text-2xl font-black text-rose-600 font-mono">
                {{ $summary['low_stock_count'] + $summary['out_of_stock_count'] }} <span class="text-sm font-normal text-slate-500">Produk</span>
            </div>
            <span class="text-xs text-slate-500 mt-1 block">{{ $summary['out_of_stock_count'] }} kosong, {{ $summary['low_stock_count'] }} menipis</span>
        </div>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
            <h2 class="text-base font-bold text-slate-900">Daftar Stok Produk & Evaluasi Restok</h2>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse($products as $p)
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="py-3 px-4 text-center text-xs font-mono font-bold text-slate-800">
                            ...
                            <td class="py-3 px-4 text-right text-xs text-slate-600 font-mono">
                                Rp {{ number_format($p->purchase_price, 0, ',', '.') }}
                            </td>
                            <td class="py-3 px-4 text-right font-bold text-teal-700 font-mono">
                            ...
                            <td class="py-3 px-4 text-center">
                                <a href="{{ route('restocks.create', ['product_id' => $p->id, 'quantity' => max(1, $p->min_stock - $p->stock)]) }}" 
                                   ...
                                   class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold text-teal-700 hover:text-white bg-teal-50 hover:bg-teal-600 rounded-lg border border-teal-200 transition">
                                    Restock
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="10" class="py-12 text-center text-slate-400">
                                ...
                                <p class="text-sm font-semibold text-slate-700">Tidak ada produk ditemukan sesuai filter yang dipilih.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
