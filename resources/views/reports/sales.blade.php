...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-slate-500 mb-1">
                ...
                <span>/</span>
                <span class="text-slate-800 font-semibold">Laporan Penjualan</span>
            </div>
            <h1 class="text-2xl lg:text-3xl font-black text-slate-900 tracking-tight">Laporan Penjualan & Pendapatan</h1>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
        <form method="GET" action="{{ route('reports.sales') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wider">Tanggal Mulai</label>
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wider">Tanggal Akhir</label>
            ...
            <div class="flex items-center gap-2">
                <button type="submit" 
                        class="flex-1 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-xl shadow-sm transition">
                    Terapkan Filter
                </button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            ...
            <span class="text-xs text-slate-500 mt-1 block">Dari {{ number_format($summary['total_transactions']) }} transaksi</span>
        </div>

        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Penerimaan Tunai (Cash)</span>
            <div class="mt-2 text-2xl font-black text-teal-700 font-mono">
                Rp {{ number_format($summary['cash_sales'], 0, ',', '.') }}
            </div>
            <span class="text-xs text-slate-500 mt-1 block">Kas masuk langsung</span>
        </div>

        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Penjualan Kredit (Piutang)</span>
            <div class="mt-2 text-2xl font-black text-amber-700 font-mono">
                Rp {{ number_format($summary['credit_sales'], 0, ',', '.') }}
            </div>
            <span class="text-xs text-slate-500 mt-1 block">Tercatat ke buku piutang</span>
        </div>

        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Volume Produk Terjual</span>
            <div class="mt-2 text-2xl font-black text-blue-700 font-mono">
                {{ number_format($summary['total_items_sold']) }} <span class="text-sm font-normal text-slate-500">Unit</span>
            </div>
            <span class="text-xs text-slate-500 mt-1 block">Kuantitas item keluar</span>
        </div>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
            <h2 class="text-base font-bold text-slate-900">Rincian Transaksi Penjualan</h2>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse($sales as $sale)
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="py-3 px-4">
                                @if($sale->customer)
                                    <span class="font-semibold text-slate-900">{{ $sale->customer->name }}</span>
                                    @if($sale->customer->phone)
                                        <span class="block text-[11px] text-slate-500">{{ $sale->customer->phone }}</span>
                                    @endif
                                @else
                            ...
                            <td class="py-3 px-4 text-center text-xs font-semibold text-slate-800">
                            ...
                            <td class="py-3 px-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('sales.show', $sale) }}" 
                                       title="Detail Nota"
                                       class="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-slate-100 rounded-lg transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <a href="{{ route('sales.download-pdf', $sale) }}" 
                                       ...
                                       class="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-slate-100 rounded-lg transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                        </svg>
                                    </a>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="py-12 text-center text-slate-400">
                                ...
                                <p class="text-sm font-semibold text-slate-700">Tidak ada transaksi ditemukan pada rentang filter ini.</p>
